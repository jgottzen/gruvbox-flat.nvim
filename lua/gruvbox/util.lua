...
function util.blend(fg, bg, alpha)
  local hexbg = hexToRgb(bg)
  local hexfg = hexToRgb(fg)

  local blendChannel = function(i)
    local ret = (alpha * hexfg[i] + ((1 - alpha) * hexbg[i]))
...
function util.new_color(colors, color_spec)
  if string.sub(color_spec, 1, 1) == "#" then
    ...
    -- another group
    if not colors[color_spec] then
      error("Color " .. color_spec .. " does not exist")
    end
...
function util.theme_overrides(theme, config)
  ...
  for theme_name, theme_override in pairs(config.theme) do
    if type(theme_override) ~= "table" then
      error("A theme override for " .. theme_name .. " should be a table but was " .. type(theme_override))
    end
